...
class GraphInterface:
   ...
   def delete_segment_interface(self):
       ...
       if segment_to_delete:
           # Remove from graph's segments list
           self.graph.segments.remove(segment_to_delete)


           # Update node connections if necessary
           if segment_to_delete.origin and segment_to_delete.destination:
   ...
   def show_shortest_path(self):
       ...
       # Get node objects
       origin_node = self.graph.GetNodeByName(origin)
       dest_node = self.graph.GetNodeByName(destination)
       ...
       try:
           # The error occurs here - let's directly implement the algorithm instead
           # to bypass the Path.__init__ error


           # Create a dictionary for distances and previous nodes
           distances = {}
           previous = {}
           unvisited = []
           ...
           while unvisited:
               ...
               # If we reached destination or distance is infinity (unreachable)
               if current.name == destination or distances[current] == float('inf'):
                   break
               ...
               for neighbor in current.neighbors:
                   ...
                   for segment in self.graph.segments:
                       if (segment.origin == current and segment.destination == neighbor) or \
                               (segment.destination == current and segment.origin == neighbor):
                           connecting_segment = segment
                           break


                   if connecting_segment:
                       alt_distance = distances[current] + connecting_segment.cost
           ...
           # Reconstruct path
           if previous[dest_node] is not None or origin_node == dest_node:
               # Path exists
               path_nodes = []
               current = dest_node
               ...
               # Clear plot and draw full graph
               self.plot_full_graph()


               # Draw the path
               for i in range(len(path_nodes) - 1):
                   self.ax.plot([path_nodes[i].x, path_nodes[i + 1].x],
               ...
               self.ax.plot(path_x, path_y, 'ro', markersize=8)
               ...
               total_cost = distances[dest_node]


               # Show results
               self.status_var.set(f"Path from {origin} to {destination}. Total cost: {round(total_cost, 2)}")
               messagebox.showinfo("Shortest Path",
                                   f"Path: {[node.name for node in path_nodes]}\nCost: {round(total_cost, 2)}")
           else:
               messagebox.showinfo("Result", f"No path found from {origin} to {destination}.")


       except Exception as e:
           messagebox.showerror("Error", f"Error finding path: {str(e)}")
